...
def human_eval_error_span_summary(raw):
    ...
    chunks = []
    if source_marks:
        label = ", ".join(source_preview) if source_preview else f"{len(source_marks)} mark(s)"
        chunks.append(f"source:{label}")
    if style_marks:
        label = ", ".join(style_preview) if style_preview else f"{len(style_marks)} mark(s)"
        chunks.append(f"style:{label}")
    if chunks:
...
def human_evaluation_project_detail(eval_project_id):
    ...
    project = human_eval_project_for_owner(eval_project_id, user["id"])
...
def human_evaluation(token):
    with db() as conn:
        ...
        if request.method == "POST":
            ...
            if existing:
                conn.rollback()
                flash("This text was already evaluated with this link.")
                return redirect(url_for("human_evaluation", token=token))
